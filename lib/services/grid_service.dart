...
class GridService {
  ...
  static GridResult generateGrid(GameConfig config) {
    ...
    for (var entry in allWords) {
      ...
      final result = _tryPlaceWord(grid, entry.value, size);
      if (result != null) {
    ...
    List<List<String>> finalGrid = List.generate(size, (r) {
      return List.generate(size, (c) {
        return grid[r][c] ??
            WordData.turkishAlphabet[
                _random.nextInt(WordData.turkishAlphabet.length)];
      });
    });
